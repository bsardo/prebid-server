...
type treeBuilder[T1 any, T2 any] struct {
	...
	SchemaFuncFactory rulestree.SchemaFuncFactory[T1]
	ResultFuncFactory rulestree.ResultFuncFactory[T2]
}
...
func (tb *treeBuilder[T1, T2]) Build(tree *rulestree.Tree[T1, T2]) error {
	currNode := tree.Root
	...
	for _, rule := range tb.Config.Rules {
		for ci, condition := range rule.Conditions {
			...
			if len(currNode.Children) == 0 {
				currNode.Children = make(map[string]*rulestree.Node[T1, T2], 0)
