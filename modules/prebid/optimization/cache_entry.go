...
import (
	...
	"github.com/prebid/prebid-server/v3/modules/prebid/optimization/config"
...
type cacheModelGroup[T1 any, T2 any] struct {
	...
	version      string
	analyticsKey string
	defaults     []rulestree.ResultFunction[T2] // can this be set on the tree somehow instead?
	tree         rulestree.Tree[T1, T2]
}
...
func createCacheRuleSet(cfg *config.RuleSet) (cacheRuleSet[openrtb_ext.RequestWrapper, hs.ChangeSet[hs.ProcessedAuctionRequestPayload]], error) {
	...
	for _, modelGroup := range cfg.ModelGroups {
		tree, err := rulestree.NewTree[openrtb_ext.RequestWrapper, hs.ChangeSet[hs.ProcessedAuctionRequestPayload]](
			&treeBuilder[openrtb_ext.RequestWrapper, hs.ChangeSet[hs.ProcessedAuctionRequestPayload]]{
				Config: modelGroup,
				SchemaFuncFactory: rulestree.NewRequestSchemaFunction,
